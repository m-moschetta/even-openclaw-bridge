...
function sendClear(ws) {
    ws.send(JSON.stringify({ type: 'clear' }));
}
function sendChatUpdate(ws, role, text) {
    ws.send(JSON.stringify({ type: 'chat_update', role, text }));
}
async function handleGlassesMessage(message, ws) {
    try {
        switch (message.type) {
            case 'touchbar':
...
async function handleTouchbarEvent(event, ws) {
...
async function handleActivation(ws) {
...
async function handleTranscription(ws) {
    return handleRecordingEnd(ws);
}
async function handleRecordingEnd(ws) {
    if (glassesManager.getState() !== 'LISTENING') {
